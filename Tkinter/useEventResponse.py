...
# @Author  : AeishenLin
# @File    : useEventResponse.py
# @Describe: 事件响应

# 用tkinter 可将用户事件与自定义函数绑定，用键盘或鼠标的动作事件来响应触发自定义函数的执行。其通式为：控件实例.bind(<事件代码>,<函数名>)
...
# 功能键          <Fn>例如:<F1>等
# 组合键          键名之间以减号链接,例<Control-k>,<Shift-6>,<Alt-Up>等 注意大小写
#
# 例如,将框架控件实例frame 绑定鼠标右键单击事件,调用自定义函数 myfunc()可表示为"frame.bind('<Button-3>',myfunc)",
# 注意: myfunc后面没有括号。将控件实例绑定到键盘事件和部分光标不落在具体控件实例上的鼠标事件时，
# 还需要设置该实例执行focus_set() 方法获得焦点，才能对事件持续响应。
# 例如： frame.focus_set()。所调用的自定义函数若需要利用鼠标或键盘的响应值，可将event作为参数，通过event的属性获取。event的属性见下表：
#
# event属性           意义
# x或y              相对于事件绑定控件实例左上角的坐标值(像素)
# root_x或root_y    相对于显示屏幕左上角的坐标值(像素)
